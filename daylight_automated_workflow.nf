...
process calculate_daylight_start {
    input:
    path script, stageAs: 'slurm_daylight_automated_scheduler.sh'
    ...
    script:
    """
    bash slurm_daylight_automated_scheduler.sh
    """
}
...
// Main workflow
workflow {
    cluster_options_ch = calculate_daylight_start(file('slurm_daylight_automated_scheduler.sh'))
